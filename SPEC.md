...
## 15. Production-grade MVP v2 — noise engine, triage, monetization

Adds the monetizable, production-grade layer. **Principles (override on conflict):** (1) never "compliance"/"compliant" — "issues / monitoring / regression alerts / documentation"; (2) **paywall follows marginal cost** — a client-side single scan is free forever; anything server-side, scheduled, stored, brand-carrying, or calling a paid API is paid, enforced **server-side**; (3) **signal over volume** — default views show fewer, triaged, deduplicated, plain-English issues; (4) **determinism + idempotency** — same input → same fingerprint → same issue identity.

- **§15.1 axe classification.** Capture all four axe buckets. `violations` = real issues (headline + alerts + default view). `incomplete` = **needs-review**, stored separately, **never** in the headline count or alerts, shown only in an opt-in tab. `passes`/`inapplicable` = counts only (coverage/trend).
- **§15.2 fingerprint v2 (highest-risk).** Issue identity is **page-independent**: `hash(rule_id + stable element signature [+ failure-reason])`. Element signature, best-available in order: stable `id` attr (ignore dynamic/hashed ids) → normalized ancestry (tag + *stable* class + role + key ARIA, capped depth) → accessible-name/text-hash tiebreaker. Dynamic class/id stripping via a **configurable regex list** in settings. `page_url` moves to the Occurrence (today's fingerprint bakes it in → must split).
- **§15.3 Issue + Occurrence.** Issue keyed `(project_id, rule_id, fingerprint)`, persists across scans. Occurrence `(issue_id, page_url, element_target, snippet, first_seen_scan, last_seen_scan)`. Headline counts **distinct Issues** with "appears on N pages".
- **§15.4 triage.** `Issue.status` ∈ open | in_progress | fixed | dismissed_false_positive | wont_fix. Suppression is **scoped** (occurrence | issue-in-project | rule-site-wide), **persistent** by fingerprint+scope, **attributed** (actor, timestamp, required reason). Immutable `issue_status_log` of every transition = the documentation/evidence trail.
- **§15.5 regression diff (issue-level).** New alert-worthy = fingerprint never seen AND not actively suppressed. Dismissed reappearance = suppressed (no alert). fixed→reappear = **regression** (alert + flip to open + log). Reuses the existing alert queue.
- **§15.6 default view.** open + in_progress, deduped, sorted by severity then occurrence count. Hidden/toggleable: needs-review, dismissed, wont-fix, fixed.
- **§15.7 entitlements.** `accounts` (org/billing owner; 1 user→1 account for MVP) with `plan` (free|solo|agency|agency_plus) + `status`. **Every paid endpoint checks entitlement server-side** (402/403). `usage_meters` per (account, metric, period) with per-tier caps → clear limit/upgrade responses. Stripe checkout link + webhook sets entitlement.
- **§15.8 white-label report.** Server-side **PDF** (Playwright `page.pdf()` on the report HTML), sourced from **deduped Issues**, dismissed excluded (optional "reviewed & dismissed" count), needs-review separate, branded + timestamped, no "compliant" copy. Entitlement-gated.
- **§15.9 AI fixes.** Free = axe `help`/`help_url` passthrough (done). Paid = LLM explanation, usage-metered + capped, **cached by `(rule_id, fingerprint)`**. Gated.
- **§15.10 statement generator.** Branded accessibility statement from latest scan + cadence; **no conformance/compliance claims**. Gated. Low priority.

**Data model additions (§5):** `issues`, `occurrences`, `issue_status_log`, `suppressions`, `needs_review`, `accounts`, `subscriptions`/entitlements, `usage_meters`, `ai_explanations_cache`; `account_id` on projects; branding re-scopable to account. Build order + stage gates live in `PLAN.md` (S1–S7).
